...
class PlayService:
    ...
    def start_or_resume_game(self, user_id: int, gameset_id : int, dailygame: bool = False) -> PlayResult:
        try:
            ...
            logger.error(f"Error while starting or resuming game: {e}")
            raise HTTPException(status_code=500, detail=f"Error while starting or resuming game: {e}")
    ...
    def play_words(self, game_id, user, played_words) -> PlayResult:
        try:
            ...
            logger.error(f"Error while playing words: {e}", exc_info=True)
            raise HTTPException(status_code=500, detail=f"Error while playing words: {e}")
    ...
    def get_daily_play_gameset(self, user_id: int) -> PlayGameSet:
        try:
            ...
            logger.error(f"Error while playing daily gameset: {e}", exc_info=True)
            raise HTTPException(status_code=500, detail="Error while playing daily gameset")
