...
def get_all_users(user_service: UserService = Depends(get_user_service),
                  current_user: UserRead = Security(get_current_user, scopes = ["user:player"])) -> list[UserRead]:
    return user_service.get_all()

@router.get("/{user_id}")
def get_user(user_id: int,
             user_service: UserService = Depends(get_user_service),
             current_user: UserRead = Security(get_current_user, scopes = ["user:player"])) -> UserRead:
    try:
